...
function SingleMovie() {
  ...
  const fetchMovieData = async () => {
    try {
      // Check if the movie exists in Firestore (user uploaded)
      ...
      if (movieDoc.exists()) {
        ...
        setMovie(movieData);
        setIsApiMovie(false); 
        console.log(movieData); // Log dữ liệu phim từ Firestore
        // It's a user-uploaded movie
      } else {
        // If the movie doesn't exist in Firebase, fetch from TMDB API
        ...
        setMovie(apiData);
        setIsApiMovie(true); // It's a TMDB movie
        console.log(apiData); // Log dữ liệu phim từ API
      }

      // Fetch recommendations (user uploaded movies)
  ...
  const handlePlay = () => {
    setPlay(true);
    addRecently(movie); // Add to recently watched
  };
          ...
          {play ? (
            isApiMovie ? (
              ...
              <iframe
                ...
                height="620"
                allowFullScreen
                src={movie.video} // Assuming movie.video contains the video URL for user-uploaded movies
                autoPlay
              />
            )
          ) : (
            <div className="w-full h-screen rounded-lg overflow-hidden relative">
              ...
              <img
                src={isApiMovie 
                  ...
                  : movie.backdropUrl} // Assuming backdropUrl contains the URL for the user-uploaded movie
                alt={movie?.title}
        ...
        <div className="flex justify-between mx-20">
          <div className="flex flex-col w-1/2 mb-15">
            ...
            <div className="flex items-center gap-6">
              <div className="w-[166px] h-[54px] bg-[#2C2C2C] text-white rounded-md flex items-center justify-center gap-3 mb-4">
                ...
                <p className="font-bold text-xl ">
                  {isApiMovie ? movie.vote_average : movie.voteAverage}
                </p>
                <p className="size-6 text-gray-500">
                  ({isApiMovie ? movie.vote_count : movie.voteCount})
                </p>
              </div>
            ...
            <div className="mb-4 flex">
              <IoIosRadioButtonOn />
              <span className="text-sm font-medium ">
                {isApiMovie ? movie.genres.map(genre => genre.name).join(', ') 
                : movie.genres.join(', ')}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <p className="font-medium">
                {isApiMovie 
                  ...
                  : movie.cast?.slice(0, 2).map(actor => actor.name).join(', ') || "Không có thông tin"
                }
              </p>
            </div>
